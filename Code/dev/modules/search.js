...
module.exports.search = async (req, res, searchTerm) => {
  async function languages(searchTerm) {
    ...
    let join = `join users on languages.fk_user_id=users.user_id`;

    let sql = `SELECT * FROM languages ${join} WHERE ${language} OR ${title} OR ${description} OR ${tags}  ORDER BY language`;
  ...
  async function frameworks(searchTerm) {
    ...
    let description = `description LIKE '%${searchTerm}%'`;
    let tags = `tags LIKE '%${searchTerm}%'`;
    let join = `join users on frameworks.fk_user_id=users.user_id`;

    let sql = `SELECT * FROM frameworks ${join} WHERE ${framework} OR ${title} OR ${description} OR ${tags}  ORDER BY framework`;
  ...
  async function tools(searchTerm) {
    ...
    let description = `description LIKE '%${searchTerm}%'`;
    let tags = `tags LIKE '%${searchTerm}%'`;
    let join = `join users on tools.fk_user_id=users.user_id`;

    let sql = `SELECT * FROM tools ${join} WHERE ${framework} OR ${title} OR ${description} OR ${tags}  ORDER BY tool`;
